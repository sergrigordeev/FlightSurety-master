...
contract FlightSuretyApp {
    ...
    {
        address[] storage votesForAirline = votesMap[airlineAddress];
        success = false;
        votes = votesForAirline.length;
        uint256 count = dataContract.airlineCount();

        bool voted = false;
        for (uint256 idx = 0; idx < votesForAirline.length; idx++) {
            if (votesForAirline[idx] == msg.sender) {
                voted = true;
                break;
            }
        }
        ...
        return (success, votesForAirline.length);
    }
